...
def test_fts5_columns(note_repository):
    ...
    assert any(row[0] == 999999 for row in result), (
        "Inserted FTS5 row with rowid 999999 should be matchable"
    )
...
def test_fts5_trigger_sync_on_insert(note_repository):
    ...
    assert len(result) == 1, (
        f"Expected 1 FTS5 row for trigger-synced note, got {len(result)}"
    )
...
def test_fts5_populated_after_rebuild(note_repository):
    ...
    with note_repository.session_factory() as session:
        results = session.execute(
            text(
                "SELECT rowid FROM notes_fts WHERE notes_fts MATCH 'antidisestablishmentarianism'"
            )
        ).fetchall()

    assert len(results) == 1, (
        f"Expected 1 FTS5 result after rebuild, got {len(results)}"
    )
