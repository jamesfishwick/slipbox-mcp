...
def test_get_note(zettel_service):
    ...
    assert {tag.name for tag in retrieved.tags} == {"service", "get"}, (
        f"Tag mismatch: {{{', '.join(t.name for t in retrieved.tags)}}}"
    )
...
def test_update_note(zettel_service):
    ...
    assert "This note has been updated through the service." in updated.content, (
        f"Expected update text in content: {updated.content!r}"
    )
    assert {tag.name for tag in updated.tags} == {"service", "updated"}, (
        f"Tag mismatch: {{{', '.join(t.name for t in updated.tags)}}}"
    )
...
def test_delete_note(zettel_service):
    ...
    assert zettel_service.get_note(note.id) is not None, (
...
def test_create_link(zettel_service):
    ...
    assert source.links[0].target_id == target_note.id, (
        f"Forward link target mismatch: {source.links[0].target_id!r}"
    )
    assert source.links[0].link_type == LinkType.REFERENCE, (
        f"Forward link type mismatch: {source.links[0].link_type!r}"
    )
    assert source.links[0].description == "A test link via service", (
        f"Forward link description mismatch: {source.links[0].description!r}"
    )

    # Assert -- inverse link
    assert len(target.links) == 1, (
        f"Target should have 1 inverse link, got {len(target.links)}"
    )
    assert target.links[0].target_id == source_note.id, (
        f"Inverse link target mismatch: {target.links[0].target_id!r}"
    )
    assert target.links[0].link_type == LinkType.REFERENCE, (
        f"Inverse link type mismatch: {target.links[0].link_type!r}"
    )
    ...
    assert outgoing[0].id == target_note.id, (
        f"Outgoing note ID mismatch: {outgoing[0].id!r}"
    )
    ...
    assert incoming[0].id == source_note.id, (
        f"Incoming note ID mismatch: {incoming[0].id!r}"
    )

    both = zettel_service.get_linked_notes(source_note.id, "both")
    assert len(both) == 1, f"Expected 1 'both' link, got {len(both)}"
    assert both[0].id == target_note.id, (
        f"Both-direction note ID mismatch: {both[0].id!r}"
    )
...
def test_search_notes(zettel_service):
    ...
    assert len(python_notes) == 2, (
        f"Expected 2 python-tagged notes, got {len(python_notes)}"
    )
    assert {n.id for n in python_notes} == {note1.id, note2.id}, (
        f"Python notes ID mismatch: {{{', '.join(n.id for n in python_notes)}}}"
    )
    ...
    assert "newTag" in {tag.name for tag in updated.tags}, (
        "newTag should be present after add"
    )

    zettel_service.remove_tag_from_note(first.id, "newTag")
    updated = zettel_service.get_note(first.id)
    assert "newTag" not in {tag.name for tag in updated.tags}, (
        "newTag should be gone after remove"
    )
...
def test_export_note_returns_yaml_frontmatter(zettel_service):
    ...
    assert post.metadata.get("type") == "permanent", (
        f"Expected type 'permanent', got {post.metadata.get('type')!r}"
    )
    assert set(post.metadata.get("tags", [])) == {"export", "test"}, (
        f"Tag mismatch: {post.metadata.get('tags')!r}"
    )
    assert post.metadata.get("references") == [REF], (
        f"References mismatch: {post.metadata.get('references')!r}"
    )
    assert "Export Test" in post.content, (
        f"Expected 'Export Test' in content: {post.content[:100]!r}"
    )
...
def test_export_note_includes_links_section(zettel_service):
    ...
    assert f"supports [[{target.id}]]" in result, (
        f"Expected supports link in export: {result[:200]!r}"
    )
    assert "key evidence" in result, (
        f"Expected 'key evidence' in export: {result[:200]!r}"
    )
...
def test_export_note_round_trips(zettel_service):
    ...
    assert parsed.note_type == NoteType.PERMANENT, (
        f"Round-trip note type mismatch: {parsed.note_type!r}"
    )
    assert {t.name for t in parsed.tags} == {"round-trip", "test"}, (
        f"Round-trip tag mismatch: {{{', '.join(t.name for t in parsed.tags)}}}"
    )
    assert parsed.references == [REF], (
        f"Round-trip references mismatch: {parsed.references!r}"
    )
    assert len(parsed.links) == 1, (
        f"Expected 1 link after round-trip, got {len(parsed.links)}"
    )
    assert parsed.links[0].target_id == target.id, (
        f"Round-trip link target mismatch: {parsed.links[0].target_id!r}"
    )
    assert parsed.links[0].description == "key link", (
        f"Round-trip link description mismatch: {parsed.links[0].description!r}"
    )
