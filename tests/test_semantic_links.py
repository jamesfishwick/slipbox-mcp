...
class TestSemanticLinks:
    ...
    def test_all_link_types_creation(self, zettel_service):
        ...
        for i, (link_type, description) in enumerate(link_types):
            ...
            assert len(matching_links) == 1, (
                f"Expected 1 matching link for {link_type.value}, got {len(matching_links)}"
            )
            assert matching_links[0].link_type == link_type, (
                f"Expected link type {link_type.value}, got {matching_links[0].link_type.value}"
            )
            assert matching_links[0].description == description, (
                f"Expected description '{description}', got '{matching_links[0].description}'"
            )
            ...
            assert len(matching_links) == 1, (
                f"Expected 1 persisted link for {link_type.value}, got {len(matching_links)}"
            )
            assert matching_links[0].link_type == link_type, (
                f"Expected persisted link type {link_type.value}, got {matching_links[0].link_type.value}"
            )
            assert matching_links[0].description == description, (
                f"Expected persisted description '{description}', got '{matching_links[0].description}'"
            )
    ...
    def test_bidirectional_semantic_links(self, zettel_service):
        ...
        for i, (source_type, expected_inverse) in enumerate(inverse_pairs):
            ...
            assert len(source_links) == 1, (
                f"Expected 1 outgoing link for {source_type.value}, got {len(source_links)}"
            )
            assert source_links[0].link_type == source_type, (
                f"Expected source link type {source_type.value}, got {source_links[0].link_type.value}"
            )
            ...
            assert len(target_links) == 1, (
                f"Expected 1 inverse link for {source_type.value}, got {len(target_links)}"
            )
            assert target_links[0].link_type == expected_inverse, (
                f"Expected inverse type {expected_inverse.value}, got {target_links[0].link_type.value}"
            )
    ...
    def test_links_persistence_through_save_and_load(self, zettel_service):
        ...
        assert len(matching_links) == 1, (
            f"Expected 1 link after load, got {len(matching_links)}"
        )
        assert matching_links[0].link_type == LinkType.REFINES, (
            f"Expected REFINES after load, got {matching_links[0].link_type.value}"
        )
        assert (
            matching_links[0].description == "Testing persistence of semantic links"
        ), f"Expected persistence description, got '{matching_links[0].description}'"
    ...
    def test_multiple_link_types_between_notes(self, zettel_service):
        ...
        link_types = [LinkType.EXTENDS, LinkType.SUPPORTS, LinkType.QUESTIONS]
        ...
        assert len(target_links) == len(link_types), (
            f"Expected {len(link_types)} links, got {len(target_links)}"
        )

        # Verify each link type is present
        for link_type in link_types:
            assert any(link.link_type == link_type for link in target_links), (
                f"Link type {link_type.value} not found among target links"
            )
    ...
    def test_updating_link(self, zettel_service):
        ...
        assert len(matching_links) == 1, (
            f"Expected 1 updated link, got {len(matching_links)}"
        )
        assert matching_links[0].link_type == LinkType.EXTENDS, (
            f"Expected EXTENDS after update, got {matching_links[0].link_type.value}"
        )
        assert matching_links[0].description == "Updated link description", (
            f"Expected updated description, got '{matching_links[0].description}'"
        )
        ...
        assert len(matching_links) == 1, (
            f"Expected 1 persisted updated link, got {len(matching_links)}"
        )
        assert matching_links[0].link_type == LinkType.EXTENDS, (
            f"Expected EXTENDS after reload, got {matching_links[0].link_type.value}"
        )
        assert matching_links[0].description == "Updated link description", (
            f"Expected updated description after reload, got '{matching_links[0].description}'"
        )
    ...
    def test_removing_links(self, zettel_service):
        ...
        assert len(target_links) == len(link_types), (
            f"Expected {len(link_types)} links before removal, got {len(target_links)}"
        )
        ...
        assert len(target_links) == len(link_types) - 1, (
            f"Expected {len(link_types) - 1} links after removal, got {len(target_links)}"
        )
        assert not any(link.link_type == LinkType.SUPPORTS for link in target_links), (
            "SUPPORTS link should have been removed"
        )
        assert any(link.link_type == LinkType.EXTENDS for link in target_links), (
            "EXTENDS link should still be present after removing SUPPORTS"
        )
        assert any(link.link_type == LinkType.QUESTIONS for link in target_links), (
            "QUESTIONS link should still be present after removing SUPPORTS"
        )

        # Remove all remaining links to target
        zettel_service.remove_link(source_id=source_note.id, target_id=target_note.id)
        ...
        assert len(target_links) == 0, (
            f"Expected 0 links after removing all, got {len(target_links)}"
        )
    ...
    def test_bidirectional_link_removal(self, zettel_service):
        ...
        source, target = zettel_service.remove_link(
            source_id=source_note.id, target_id=target_note.id, bidirectional=True
        )
...
"""
        ...
        assert link_types["20250101120001"] == LinkType.EXTENDS, (
            f"Expected EXTENDS for 20250101120001, got {link_types.get('20250101120001')}"
        )
        assert link_types["20250101120002"] == LinkType.CONTRADICTS, (
            f"Expected CONTRADICTS for 20250101120002, got {link_types.get('20250101120002')}"
        )
        assert link_types["20250101120003"] == LinkType.SUPPORTS, (
            f"Expected SUPPORTS for 20250101120003, got {link_types.get('20250101120003')}"
        )
    ...
    def test_markdown_generation_with_links(self, zettel_service):
        ...
        assert "- extends [[target123]] Extends link" in markdown, (
            f"Generated markdown missing expected extends link line, got: {markdown}"
        )
    ...
    def test_link_type_uniqueness_constraint(self, zettel_service):
        ...
        assert len(matching_links) == 1, (
            f"Expected 1 EXTENDS link (uniqueness constraint), got {len(matching_links)}"
        )
        # Description should not be updated since the original link is preserved
        assert matching_links[0].description == "First link", (
            f"Expected original description 'First link', got '{matching_links[0].description}'"
        )
    ...
    def test_get_linked_notes_outgoing(self, zettel_service):
        ...
        target_notes = []
        link_types = [LinkType.EXTENDS, LinkType.REFINES, LinkType.SUPPORTS]
        ...
        assert len(linked_notes) == len(link_types), (
            f"Expected {len(link_types)} outgoing linked notes, got {len(linked_notes)}"
        )
        ...
        for target in target_notes:
            assert target.id in linked_ids, (
                f"Target note {target.id} not found in outgoing linked notes"
            )
    ...
    def test_mcp_get_linked_notes_tool(self, zettel_service):
        ...
        slipbox_get_linked_notes = server.mcp._tool_manager.get_tool(
            "slipbox_get_linked_notes"
        ).fn
        result = slipbox_get_linked_notes(note_id=central_note.id, direction="outgoing")
        ...
        assert "linked notes for " + central_note.id in result, (
            f"Expected note ID {central_note.id} in MCP result, got: {result[:200]}"
        )
        assert "Link type: extends" in result, (
            f"Expected 'Link type: extends' in MCP result, got: {result[:200]}"
        )
        assert "Testing extends relationship" in result, (
            f"Expected link description in MCP result, got: {result[:200]}"
        )
    ...
    def test_mcp_create_link_tool(self, zettel_service):
        ...
        # Access the tool function via MCP's tool registry
        slipbox_create_link = server.mcp._tool_manager.get_tool(
            "slipbox_create_link"
        ).fn
        ...
        assert "Bidirectional link created" in result, (
            f"Expected 'Bidirectional link created' in MCP result, got: {result[:200]}"
        )
        ...
        assert len(source_links) == 1, (
            f"Expected 1 source link via MCP tool, got {len(source_links)}"
        )
        assert source_links[0].link_type == LinkType.SUPPORTS, (
            f"Expected SUPPORTS via MCP tool, got {source_links[0].link_type.value}"
        )
        assert (
            source_links[0].description == "Testing supports relationship via MCP tool"
        ), f"Expected MCP tool description, got '{source_links[0].description}'"
        ...
        assert len(target_links) == 1, (
            f"Expected 1 inverse link via MCP tool, got {len(target_links)}"
        )
        assert target_links[0].link_type == LinkType.SUPPORTED_BY, (
            f"Expected SUPPORTED_BY inverse via MCP tool, got {target_links[0].link_type.value}"
        )
    ...
    def test_link_cycle_detection(self, zettel_service):
        ...
        assert a_outgoing[0].link_type == LinkType.EXTENDS, (
            f"Expected A->B EXTENDS, got {a_outgoing[0].link_type.value}"
        )
        ...
        assert b_outgoing[0].link_type == LinkType.REFINES, (
            f"Expected B->C REFINES, got {b_outgoing[0].link_type.value}"
        )
        ...
        assert c_outgoing[0].link_type == LinkType.CONTRADICTS, (
            f"Expected C->A CONTRADICTS, got {c_outgoing[0].link_type.value}"
        )
        ...
        assert note_b.id in a_linked_ids, (
            f"Note B ({note_b.id}) not found in A's linked notes"
        )
        assert note_c.id in a_linked_ids, (
            f"Note C ({note_c.id}) not found in A's linked notes"
        )
    ...
    def test_semantic_links_in_search_results(self, zettel_service):
        ...
        assert len(linked_notes) == 1, (
            f"Expected 1 outgoing linked note from hub, got {len(linked_notes)}"
        )
        assert linked_notes[0].id == linked_note.id, (
            f"Expected linked note ID {linked_note.id}, got {linked_notes[0].id}"
        )

        # Search for notes by tag to verify all test notes are found
        search_results = zettel_service.get_notes_by_tag("search")
        assert len(search_results) == 3, (
            f"Expected 3 notes with 'search' tag, got {len(search_results)}"
        )
    ...
    def test_find_similar_notes_with_semantic_links(self, zettel_service):
        ...
        assert target1.id in similar_ids, (
            f"Target1 ({target1.id}) not found in similar notes"
        )
        assert target2.id in similar_ids, (
            f"Target2 ({target2.id}) not found in similar notes"
        )
        ...
        assert len(similar_notes) >= 2, (
            f"Expected at least 2 similar notes, got {len(similar_notes)}"
        )
    ...
    def test_central_notes_with_semantic_links(self, zettel_service):
        ...
        assert central_dict[hub.id] >= 3, (
            f"Expected hub to have >= 3 connections, got {central_dict[hub.id]}"
        )
        ...
        if satellite1_position is not None:
            assert hub_position < satellite1_position, (
                f"Hub (position {hub_position}) should be ranked higher than satellite1 (position {satellite1_position})"
            )

        # Orphan should not be in central notes
        assert orphan.id not in central_dict, (
            f"Orphan note ({orphan.id}) should not appear in central notes"
        )
