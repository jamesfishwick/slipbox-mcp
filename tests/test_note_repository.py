...
def test_create_note_returns_note_with_id_and_all_fields(note_repository):
    ...
    # Arrange
    note = make_note(
        title="Test Note", content="This is a test note.", tags=["test", "example"]
    )
    ...
    assert saved.title == "Test Note", (
        f"Expected title 'Test Note', got {saved.title!r}"
    )
    assert saved.content == "This is a test note.", (
        f"Expected content 'This is a test note.', got {saved.content!r}"
    )
    assert saved.note_type == NoteType.PERMANENT, (
        f"Expected PERMANENT note type, got {saved.note_type}"
    )
...
def test_get_note_returns_file_content_with_title_header(note_repository):
    ...
    assert retrieved.title == "Get Test Note", (
        f"Expected title 'Get Test Note', got {retrieved.title!r}"
    )
...
def test_update_note_persists_new_title_content_and_tags(note_repository):
    ...
    # Arrange
    saved = note_repository.create(
        make_note(title="Original", content="Original body.", tags=["old"])
    )
    ...
    assert retrieved.title == "Updated Title", (
        f"Expected title 'Updated Title', got {retrieved.title!r}"
    )
    assert retrieved.content.strip() == EXPECTED_CONTENT.strip(), (
...
def test_delete_note_removes_from_repository(note_repository):
    ...
    # Assert
    assert note_repository.get(saved.id) is None, (
        f"Note {saved.id} should be gone after delete()"
    )
...
def test_search_by_content_finds_matching_notes(note_repository):
    ...
    # Arrange
    python = note_repository.create(
        make_note(
            title="Python Programming", content="Python is versatile.", tags=["python"]
        )
    )
    data_sci = note_repository.create(
        make_note(
            title="Data Science", content="Data science uses Python.", tags=["python"]
        )
    )
    js = note_repository.create(
        make_note(
            title="JavaScript Basics",
            content="JavaScript for web.",
            tags=["javascript"],
        )
    )
    ...
    assert python.id in result_ids, (
        "Python Programming note should match content='Python'"
    )
...
def test_search_by_title_finds_exact_title_match(note_repository):
    ...
    js = note_repository.create(
        make_note(title="JavaScript Basics", tags=["javascript"])
    )
    ...
    assert len(results) == 1, (
        f"Expected 1 result, got {len(results)}: {[n.title for n in results]}"
    )
    assert results[0].id == js.id, (
        f"Expected JavaScript note ID {js.id}, got {results[0].id}"
    )
...
def test_search_by_note_type_filters_correctly(note_repository):
    ...
    note_repository.create(
        make_note(title="Permanent Note", note_type=NoteType.PERMANENT)
    )
    structure = note_repository.create(
        make_note(title="Structure Note", note_type=NoteType.STRUCTURE)
    )
    ...
    assert results[0].id == structure.id, (
        f"Expected structure note ID {structure.id}, got {results[0].id}"
    )
...
def test_find_by_tag_returns_all_tagged_notes(note_repository):
    ...
    # Arrange
    python = note_repository.create(
        make_note(title="Python", tags=["python", "programming"])
    )
    js = note_repository.create(
        make_note(title="JavaScript", tags=["javascript", "programming"])
    )
    note_repository.create(make_note(title="Data Science", tags=["data"]))
...
def test_references_survive_get_all(note_repository):
    ...
    created = note_repository.create(
        make_note(title="Bulk Ref Note", references=[LUHMANN_REF])
    )
...
def test_references_survive_search(note_repository):
    ...
    # Assert
    assert len(results) == 1, (
        f"Expected 1 result for 'Search Ref Note', got {len(results)}"
    )
...
def test_references_survive_update(note_repository):
    ...
    # Assert
    assert len(results) == 1, (
        f"Expected 1 result for 'Update Ref Note', got {len(results)}"
    )
...
def test_create_link_between_notes_is_persisted(note_repository):
    ...
    # Act
    source.add_link(
        target_id=target.id, link_type=LinkType.REFERENCE, description="A test link"
    )
    updated = note_repository.update(source)

    # Assert — link on returned note
    assert len(updated.links) == 1, (
        f"Expected 1 link on source, got {len(updated.links)}"
    )
    link = updated.links[0]
    assert link.target_id == target.id, (
        f"Expected link target {target.id}, got {link.target_id}"
    )
    assert link.link_type == LinkType.REFERENCE, (
        f"Expected REFERENCE link type, got {link.link_type}"
    )
    assert link.description == "A test link", (
        f"Expected description 'A test link', got {link.description!r}"
    )
    ...
    assert linked[0].id == target.id, (
        f"Expected linked note {target.id}, got {linked[0].id}"
    )
...
def test_get_all_includes_refless_literature_via_model_construct_fallback(
    note_repository,
):
