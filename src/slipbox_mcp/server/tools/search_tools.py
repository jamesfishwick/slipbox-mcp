...
def register_search_tools(server) -> None:
    ...
    def slipbox_list_notes_by_date(
        ...
        Args:
            ...
            limit: Maximum results (default: 10)
        """
        # Only the ISO date parsing gets a bespoke message; any other failure
        # falls through to the shared error handler on the decorator.
        try:
            start_datetime = (
                datetime.fromisoformat(f"{start_date}T00:00:00") if start_date else None
            )
            end_datetime = (
                datetime.fromisoformat(f"{end_date}T23:59:59") if end_date else None
            )
